...
public final class DisplayLegacy1 {
    ...
    public static void setup(String name,int width, int height){
        DisplayLegacy1.height=height;
        DisplayLegacy1.width=width;
        DisplayLegacy1.name=name;
    }
    
    public static void start(){
        System.out.println(name+" "+width+" "+height);
        DisplayLegacy1.frame = new JFrame(name);
        System.out.println(frame.getBounds());
        DisplayLegacy1.frame.setSize(width,height);
        DisplayLegacy1.frame.setVisible(true);
        DisplayLegacy1.frame.setLayout(null);
        DisplayLegacy1.frame.setDefaultCloseOperation(WindowConstants.EXIT_ON_CLOSE);
    ...
    public static void clearDisplay(){
        for (Component component: DisplayLegacy1.frame.getComponents()) {
            DisplayLegacy1.frame.remove(component);
        }
    }
        ...
        for (int layer = 0; layer < 10; layer++) {
            for (AbstractTile tile:activeTiles) {
                if(tile.getLayer()==layer){
                    ...
                    DisplayLegacy1.frame.add(tile);
                    tile.setVisible(true);
                }
            }
        }
        DisplayLegacy1.frame.update(DisplayLegacy1.frame.getGraphics());
    }
}
